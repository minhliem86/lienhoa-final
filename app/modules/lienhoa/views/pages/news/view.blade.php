...
<section class="content">
	<div class="box">
		{{Form::model($news,array('route'=>array('admin.tintuc.update',$news->id),'method'=>'PUT' ,'class'=>'formAdmin form-horizontal','files'=>true))}}
			...
			<div class="form-margin">
				<span class="inline-radio"><input type="radio" name="status" value="1" {{$news->status == 1 ? 'checked' : ''}}> <b>Hiện</b> </span>
				<span class="inline-radio"><input type="radio" name="status" value="0" {{$news->status == 0 ? 'checked' : ''}}> <b>Ẩn</b> </span>
			</div>
			<div class="form-margin">
				<button type="button" id="open_img" class="btn btn-primary" onclick="openCKFinder()">Choose images</button>
				<div id="preview_img">
					<img src="{{asset($news->image_path)}}" id="pre" class="img-responsive" style="max-width:250px" />
				</div>
				{{Form::hidden('img-bk',$news->image_path, array('class'=>'form-control'))}}
			</div>
